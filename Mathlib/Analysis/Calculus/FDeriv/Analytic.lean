...
protected theorem FormalMultilinearSeries.fderiv_sum [CompleteSpace F] (h : ‖x‖ₑ < p.radius) :
  ...
  have h := p.hasFPowerSeriesOnBall (lt_of_le_of_lt (by simp) h) |>.fderiv.hasSum
    (show x ∈ Metric.eball 0 p.radius by simpa using h) |>.tsum_eq.symm
  rw [zero_add] at h
...
protected theorem FormalMultilinearSeries.hasFDerivAt_sum [CompleteSpace F] (h : ‖x‖ₑ < p.radius) :
  ...
  exact p.hasFPowerSeriesOnBall (lt_of_le_of_lt (by simp) h)
